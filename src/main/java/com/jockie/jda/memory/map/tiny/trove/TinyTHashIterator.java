 ...
 */
public abstract class TinyTHashIterator<V> implements TIterator, Iterator<V> {


    private final TinyTObjectHash<V> _object_hash;

    /** the data structure this iterator traverses */
    protected final TinyTHash _hash;
     ...
     */
    protected TinyTHashIterator( TinyTObjectHash<V> hash ) {
        _hash = hash;
    ...
    protected final int nextIndex() {
        ...
        int i = _index;
        while ( i-- > 0 && ( set[i] == TinyTObjectHash.FREE || set[i] == TinyTObjectHash.REMOVED ) ) {
            ;
        }
        return i;
    }
